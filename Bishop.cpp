...
/**
 * the bishop's piece move ,check whether or not it's legal to the bishop move to the pos that was entered by the user,
 ...
 */
bool Bishop::move( int currentCol, int currentRow, int futureCol, int futureRow)
{
    for (int i = 1; i < 8; ++i)
    {
        if ((futureCol == currentCol + i && futureRow == currentRow + i) || (futureCol == currentCol - i &&
